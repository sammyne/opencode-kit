...
本仓库是 OpenCode 工作流的 agent 和 skill 集合（opencode-kit）。没有应用代码，没有构建/测试/lint 命令。产出物是 `.opencode/agents/` 下的三个 agent 配置和 `.opencode/skills/` 下的 skill，设计为拷贝到其他项目中使用。
...
.opencode/skills/
  retro/SKILL.md — 工程复盘，分析 git 历史生成复盘报告（输出到项目根目录 retro-*.md）
```

**工作流**：用户激活 planner → 生成 `requirements.md` + `todo.md` 到 `.opencode/plans/{名称}/` → 用户确认 → 切换到 builder agent → 按依赖顺序执行任务 → 可选：切换到 reviewer agent 审查代码变更 → 可选：加载 retro skill 生成工程复盘。
...
- **retro skill** 由任意 agent 按需加载，继承调用者的权限；独立于其他 agent，随时可运行
- 三个 agent 均从**目标项目**（即 agent 文件被拷贝到的项目）的 `AGENTS.md` 读取项目结构信息，而非本仓库的 `AGENTS.md`
